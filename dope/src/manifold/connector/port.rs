...
impl<'d, B: AsRef<[u8]>> Entry<'d, B> {
    ...
    fn deactivate(&self, token: Token) {
        ...
        self.close.set(false);
        let detached = self.queue.entries.detach_all();
        drop(detached);
    }
...
impl<'d, B: AsRef<[u8]>> Port<'d, B> {
    ...
    ) -> Option<super::Requests> {
        self.with_receiver(token, |receiver| {
            receiver.drain(push);
            super::Requests {
                shutdown: None,
                // A port-initiated close is always recoverable (the port carries
                // no terminal/transient distinction); permanent close is reached
                // by raw `ConnApp`s via `CloseKind::Permanent`.
                close: receiver.take_close().then_some(super::CloseKind::Reconnect),
            }
        })
    }
}
