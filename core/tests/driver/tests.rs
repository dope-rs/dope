...
use dope_core::platform::{Mismatch, Platform, Snapshot};
...
fn interval_emits_repeated_completions_and_can_be_cancelled() {
    with_driver(|mut driver| {
        ...
        let mut ticks = 0;
        let mut completions = [dope_core::io::Cqe::ZERO; 16];
        while ticks < 2 && Instant::now() < deadline {
            ...
            ticks += completions[..n]
                .iter()
                .filter(|cqe| {
                    Token::try_from_raw(cqe.user_data)
                        .is_some_and(|token| token.same_target(target()))
                        && cqe.result != -libc::ECANCELED
                })
                .count();
        }
        ...
        #[cfg(target_os = "linux")]
        {
            ...
            while !cancelled && Instant::now() < deadline {
                ...
                let n = driver.drain(&mut completions);
                cancelled = completions[..n].iter().any(|cqe| {
                    Token::try_from_raw(cqe.user_data)
                        .is_some_and(|token| token.same_target(target()))
                        && cqe.result == -libc::ECANCELED
                });
...
fn saturated_snapshot() -> Snapshot {
    let mut snap = Driver::snapshot().expect("detect");
...
fn snapshot_detect_ok() {
    let snap = Driver::snapshot().expect("detect");
...
fn driver_does_not_raise_nofile() {
    let before = Driver::snapshot().expect("detect before").rlimit_nofile;
    let driver = Driver::new(driver::Config::for_quic_udp(1, 8)).expect("driver");
    let after = Driver::snapshot().expect("detect after").rlimit_nofile;
