...
use dope::{Cqe, Dispatcher, Drive, Idle, runtime::executor::Session};
...
pub fn block_on<'d, D: Dispatcher<'d>, F: Future>(
    sess: &mut Session<'d>,
    ...
    loop {
        let n = Drive::drain(driver, &mut buf);
        for cqe in &buf[..n] {
        ...
        if one_shot.as_ref().is_done() {
            // flush SQEs the final turn queued (a closing conn pushes close/cancel)
            let _ = driver.park(Duration::ZERO);
            return one_shot.take_output().expect("OneShot reported done");
        }
